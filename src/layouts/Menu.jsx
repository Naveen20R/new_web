    ...
    return (
        <section className='d-none d-md-block container-fluid bg-light '>
            <nav className='container w-100 text-center mb-1 '>
                <ul className=' list-unstyled position-sticky z-3 d-flex justify-content-center gap-2 flex-wrap align-items-start ul-parent'>
                    {mainCategories.map((category) => (
                        <>
                            {!category.child ?
                                (
                                    <li className='px-4 py-2 rounded list-item li-child' key={category.id}>
                                    ...
                                    <li className='px-4 py-2 rounded-top list-item d-flex align-items-center flex-column li-child'
                                        ...
                                        {(activeMenu === category.id) && (
                                            <ul className='bg-white shadow rounded-bottom border-top border-2 border-black  container position-absolute start-0 top-100 py-2 list-unstyled ul-child' style={{ minHeight: '140px' }}>
                            ...
                            {(activeMenu === 5) && (
                                <ul className='bg-white shadow rounded-bottom border-top border-2 border-black  container position-absolute start-0 top-100 py-2 list-unstyled ul-child' style={{ minHeight: '140px' }}>
                                    {moreCategories.map((category) => (
                                        <>
                                            {!category.child ? (
                                                <li className='text-start row row-cols-2 more-list-item w-100 w-md-30 w-lg-25 ' key={category.id}>
                                                     <a className='nav-links rounded px-4 py-1 w-100'
                                                        href={`/${category.type2}/${category.data_query}`}
                                                        onClick={() => handleLinkClick(category.name)}
                                                    >
                                                        <img
                                                ...
                                                (
                                                    <>
                                                        <li className='text-start more-list-item d-block d-lg-flex gap-1' key={category.id}>
                                                             <a className='nav-links more-child w-100 w-md-30 w-lg-25 rounded px-4 py-1 d-flex flex-wrap' href="#" onClick={() => handleSubMenuClick(category.id)}>
                                                                <img
                                                                    className="menu-img rounded-circle me-2"
                                                                    src={
                                                                        category.image
                                                            ...
                                                            {(activeSubMenu === category.id) && (
                                                                <ul className=' list-unstyled d-flex w-100 w-lg-75 flex-wrap ps-5 ps-lg-0 align-items-center justify-content-start'>
                                                                    {category.child.map((subCategory) => (
                                                                        <li key={subCategory.id} className='more-list-item'>
                                                                             <a className='nav-links d-flex gap-2 align-items-center rounded px-4 py-1'
                                                                                href={`/${subCategory.type2}/${subCategory.data_query}`}
                                                                                onClick={() => handleLinkClick(subCategory.name)}
                                                                                style={{ fontSize: '14px' }}
